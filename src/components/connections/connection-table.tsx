...
const DEFAULT_COLUMNS: Omit<ColumnConfig, 'label'>[] = [
  ...
  },
  {
    ...
    visible: true,
    getValue: conn =>
      conn.metadata.process ? `${conn.metadata.process}${conn.metadata.uid ? `(${conn.metadata.uid})` : ''}` : '-',
  },
  {
    key: 'processPath',
    width: 250,
  ...
  const renderStatus = useCallback(
    (conn: ControllerConnectionDetail) => (
      <Badge variant='outline' className='rounded-sm gap-1.5'>
        <span className={`size-1.5 rounded-full ${conn.isActive ? 'bg-primary' : 'bg-destructive'}`} />
            ...
            {sortedConnections.map(connection => (
              ...
              >
                ...
                <td className='sticky right-1.5 bg-inherit' onClick={e => e.stopPropagation()}>
                  ...
                  >
                    {connection.isActive ? <X className='text-lg' /> : <Trash2 className='text-lg' />}
                  </Button>
                </td>
              </tr>
            ))}
