...
import { mihomoUpgradeGeo } from '@/utils/ipc';
    ...
    <SettingCard>
      <SettingItem title={t('resources.geoipDatabase')} divider>
        <div className='flex w-[70%]'>
          ...
          <Input className='h-8' value={geoipInput} onChange={event => setGeoIpInput(event.target.value)} />
        </div>
      </SettingItem>
      <SettingItem title={t('resources.geositeDatabase')} divider>
        <div className='flex w-[70%]'>
          ...
          <Input className='h-8' value={geositeInput} onChange={event => setGeositeInput(event.target.value)} />
        </div>
      </SettingItem>
      <SettingItem title={t('resources.mmdbDatabase')} divider>
        <div className='flex w-[70%]'>
          ...
          <Input className='h-8' value={mmdbInput} onChange={event => setMmdbInput(event.target.value)} />
        </div>
      </SettingItem>
      <SettingItem title={t('resources.asnDatabase')} divider>
        <div className='flex w-[70%]'>
          ...
          <Input className='h-8' value={asnInput} onChange={event => setAsnInput(event.target.value)} />
        </div>
      </SettingItem>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                toast.success(t('resources.geoUpdateSuccess'));
              } catch (e) {
                toast.error(`${e}`);
              } finally {
                setUpdating(false);
          ...
          <Input
            ...
            onChange={event => {
              patchControledMihomoConfig({
                'geo-update-interval': parseInt(event.target.value, 10),
              });
            }}
          />
        </SettingItem>
