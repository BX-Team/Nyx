...
import { openFile } from '@/utils/ipc';
...
const ProfileItem: React.FC<Props> = props => {
  ...
  const { info, addProfileItem, removeProfileItem, updateProfileItem, onClick, isCurrent, switching } = props;
  ...
  const onMenuAction = async (key: string): Promise<void> => {
    switch (key) {
      ...
      case 'home': {
        if (info.home) openUrl(info.home);
        break;
      }
      case 'support': {
        if (info.supportUrl) openUrl(info.supportUrl);
        break;
      }
    }
  };
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent size='sm'>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='truncate max-w-3xs'>{info.name}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <div ref={setNodeRef} {...attributes} {...listeners} className='w-full h-full'>
          ...
          <div className='flex items-center gap-2'>
            ...
            <div className='shrink-0 -mr-1 flex items-center' onClick={e => e.stopPropagation()}>
              {info.type === 'remote' && (
                <Button size='icon-sm' variant='ghost' onClick={() => onMenuAction('update')} disabled={updating}>
                  <RefreshCcw className={cn('text-base text-muted-foreground', updating && 'animate-spin')} />
                </Button>
              )}
              <DropdownMenu>
          ...
          <div className='grid grid-cols-2 mt-2'>
            <div className='pr-3 border-r border-foreground/10 justify-items-center'>
              <div className='text-[11px] text-muted-foreground'>{t('profile.trafficRemaining')}</div>
