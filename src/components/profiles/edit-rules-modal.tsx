...
const portValidator = (value: string): boolean => {
  return /^(?:[1-9]\d{0,3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/.test(value);
};

const ipv4CIDRValidator = (value: string): boolean => {
  return /^(?:(?:[1-9]?[0-9]|1[0-9][0-9]|2(?:[0-4][0-9]|5[0-5]))\.){3}(?:[1-9]?[0-9]|1[0-9][0-9]|2(?:[0-4][0-9]|5[0-5]))(?:\/(?:[12]?[0-9]|3[0-2]))$/.test(
    value,
  );
};

const ipv6CIDRValidator = (value: string): boolean => {
  return /^([0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){7}|::|:(?::[0-9a-fA-F]{1,4}){1,6}|[0-9a-fA-F]{1,4}:(?::[0-9a-fA-F]{1,4}){1,5}|(?:[0-9a-fA-F]{1,4}:){2}(?::[0-9a-fA-F]{1,4}){1,4}|(?:[0-9a-fA-F]{1,4}:){3}(?::[0-9a-fA-F]{1,4}){1,3}|(?:[0-9a-fA-F]{1,4}:){4}(?::[0-9a-fA-F]{1,4}){1,2}|(?:[0-9a-fA-F]{1,4}:){5}:[0-9a-fA-F]{1,4}|(?:[0-9a-fA-F]{1,4}:){1,6}:)\/(?:12[0-8]|1[01][0-9]|[1-9]?[0-9])$/.test(
    value,
  );
};
...
>([
  ...
  [
    ...
    {
      name: 'PROCESS-PATH',
      example: platform === 'win32' ? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe' : '/usr/bin/wget',
    },
  ],
  [
    'PROCESS-NAME-REGEX',
...
const RuleDisplayContent: React.FC<RuleDisplayContentProps> = ({
  ...
  const logicalRuleContent =
    logicalRuleSummary.length > 0
      ...
      : rule.payload || '-';
  const logicalRuleContentTitle = logicalRuleSummary.length > 0 ? logicalRuleSummary.join(' • ') : rule.payload || '-';
  const primaryContent = isLogicalRule ? logicalRuleContent : rule.type === 'MATCH' ? rule.proxy : rule.payload;
  ...
  return (
    <>
      ...
      <div className='flex-1 min-w-0'>
        <div
          className={cn('text-sm font-medium truncate', isDeleted && 'line-through text-muted-foreground')}
          title={primaryContentTitle}
        >
          {primaryContent}
        </div>
        {rule.proxy && rule.type !== 'MATCH' && (
          <div className={cn('text-xs text-muted-foreground truncate', isDeleted && 'line-through')}>{rule.proxy}</div>
        )}
      </div>
    </>
  );
              ...
              <CommandGroup>
                {proxyGroups.map(group => (
                  ...
                  >
                    {group}
                    <CheckIcon
                      className={cn('ml-auto size-3', editingRule.proxy === group ? 'opacity-100' : 'opacity-0')}
                    />
                  </CommandItem>
                ))}
              </CommandGroup>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        !isDeleted && isCustom && 'bg-green-500/8 border-green-500/20 hover:border-green-500/40 cursor-pointer',
        !isDeleted && !isCustom && 'bg-muted/50 border-transparent hover:border-border cursor-pointer',
      ...
      <div className='flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity'>
        <Tooltip>
          ...
          <TooltipContent side='top'>
            {originalIndex !== -1 && isDeleted ? t('profile.editRules.restore') : t('profile.editRules.delete')}
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
...
const EditRulesModal: React.FC<Props> = props => {
  ...
  const customRulesCount = useMemo(() => prependRules.size + appendRules.size, [prependRules, appendRules]);
  ...
  const handleAddRule = useCallback(
    (position: 'prepend' | 'append' = 'append'): void => {
      ...
      startTransition(() => {
        ...
        if (position === 'prepend') {
          const insertPosition = newRuleItem.offset !== undefined ? Math.min(newRuleItem.offset, rules.length) : 0;

          updatedRules = [...rules];
          updatedRules.splice(insertPosition, 0, newRuleItem);

          const { newPrependRules, newAppendRules, newDeletedRules } = updateAllRuleIndicesAfterInsertion(
            prependRules,
            appendRules,
            deletedRules,
            insertPosition,
  ...
  const remapIndicesAfterMove = useCallback((set: Set<number>, oldIndex: number, newIndex: number): Set<number> => {
        ...
        <div className='h-full overflow-hidden'>
          <div className='flex gap-4 h-full'>
            ...
            <div className='w-2/5 flex flex-col gap-3 pr-1 overflow-y-auto min-h-0'>
              <div className='flex flex-col gap-3'>
                <div className='flex flex-col gap-1.5'>
                  ...
                  <Select value={newRule.type} onValueChange={value => handleRuleTypeChange(value)}>
                ...
                <div className='flex flex-col gap-1.5'>
                  ...
                  {isNewRuleLogical ? (
                    <Textarea
                      placeholder={getRuleExample(newRule.type) || t('profile.editRules.payloadPlaceholder')}
                  ...
                  {isNewRuleLogical && newRuleLogicalSummary.length > 0 && (
                    <div className='flex flex-wrap gap-1'>
                      {newRuleLogicalSummary.slice(0, 4).map((clause, idx) => (
                        <Badge key={`new-clause-${idx}`} variant='outline' className='text-[10px] max-w-65 truncate'>
                          {clause}
                        </Badge>
                      ))}
                  ...
                  {newRule.payload && newRule.type !== 'MATCH' && !isPayloadValid && (
                    <p className='text-[11px] text-destructive'>
                      {t('profile.editRules.expectedFormat') || 'Expected format'}: {getRuleExample(newRule.type)}
                    </p>
                  )}
                </div>
                  ...
                  <Popover modal open={newRuleProxyOpen} onOpenChange={setNewRuleProxyOpen}>
                    <PopoverTrigger asChild>
                      <Button variant='outline' role='combobox' className='w-full justify-between font-normal'>
                        <span className='truncate'>{newRule.proxy || t('profile.editRules.proxyPlaceholder')}</span>
                      ...
                      {isRuleSupportsNoResolve(newRule.type) && (
                        <div className='flex items-center gap-2'>
                          <Switch
                            ...
                            onCheckedChange={checked => handleAdditionalParamChange('no-resolve', checked)}
                          />
                      ...
                      {isRuleSupportsSrc(newRule.type) && (
                        <div className='flex items-center gap-2'>
                          <Switch
                            ...
                            onCheckedChange={checked => handleAdditionalParamChange('src', checked)}
                          />
              ...
              <Accordion type='single' collapsible className='mt-auto'>
                <AccordionItem value='instructions' className='border-b-0'>
                  <AccordionTrigger className='text-sm py-2'>{t('profile.editRules.instructions')}</AccordionTrigger>
                  <AccordionContent>
            ...
            <div className='w-3/5 border-l pl-4 pr-1 flex flex-col min-h-0'>
              ...
              >
                <SortableContext items={isDragEnabled ? sortableIds : []} strategy={verticalListSortingStrategy}>
