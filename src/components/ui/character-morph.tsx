...
const CharacterMorph = React.forwardRef<HTMLDivElement, CharacterMorphProps>(
  ({ texts, reserveTexts = [], className, interval = 3000, staggerDelay = 0.03, charDuration = 0.5 }, ref) => {
    const [currentIndex, setCurrentIndex] = React.useState(0);
    const currentText = texts[currentIndex] || '';
    const normalizedStaggerDelay = Math.max(0, staggerDelay);
    const maxTotalStagger = 0.24;
    const perCharDelay =
      currentText.length > 1 ? Math.min(normalizedStaggerDelay, maxTotalStagger / (currentText.length - 1)) : 0;
