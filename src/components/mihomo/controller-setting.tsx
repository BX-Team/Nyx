...
import { mihomoUpgradeUI, restartCore } from '@/utils/ipc';
import { isValidListenAddress } from '@/utils/validate';
...
const ControllerSetting: React.FC = () => {
  ...
  const upgradeUI = async (): Promise<void> => {
    try {
      ...
      toast.success(t('mihomo.controllerSettings.panelUpdateSuccess'));
    } catch (e) {
      toast.error(`${e}`);
    } finally {
      setUpgrading(false);
            ...
            <TooltipTrigger asChild>
              <Input
                ...
                onChange={event => {
                  ...
                  setExternalControllerError(r.ok ? null : (r.error ?? t('mihomo.controllerSettings.formatError')));
                }}
              />
            </TooltipTrigger>
            {externalControllerError && (
          ...
          {enableExternalUi && (
            ...
            >
              <div className='flex'>
                ...
                <Select value={externalUiUrlInput} onValueChange={value => setExternalUiUrlInput(value)}>
