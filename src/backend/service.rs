...
async fn ensure_core_binary() -> Result<std::path::PathBuf, String> {
    ...
    let app_cfg_path = dirs::app_config_path();
...
async fn ensure_runtime_config() -> Result<(std::path::PathBuf, String), String> {
    let url = manager::rebuild_config().await.map_err(|e| e.to_string())?;
    let cm = mihomo_rs::ConfigManager::with_home(dirs::data_dir()).map_err(|e| e.to_string())?;
...
fn sync_controller(url: &str, config: &std::path::Path) -> Result<(), String> {
    manager::set_controller_url(url.to_string());
    api::init_client(url, read_secret_from_config(config)).map_err(|e| e.to_string())
}

#[cfg(windows)]
fn ensure_service_installed_args() -> Result<(), String> {
    let bin_path = build_service_binpath()?;
    let action = if service_query_state()?.is_some() {
        "config"
    } else {
        "create"
    };
...
async fn send_ipc_request(req: &crate::backend::service_host::IpcRequest) -> Result<(), String> {
    ...
    if let Ok(res) = serde_json::from_str::<crate::backend::service_host::IpcResponse>(&msg) {
        match res {
            crate::backend::service_host::IpcResponse::Ok
            | crate::backend::service_host::IpcResponse::Pong => Ok(()),
            crate::backend::service_host::IpcResponse::Error { message } => Err(message),
        }
    } else {
...
async fn start_windows_service() -> Result<(), String> {
    ...
    let req = crate::backend::service_host::IpcRequest::StartCore {
...
/// Reads the controller URL from the live runtime config (if not already set),
/// initializes the API client, and probes `/version` to confirm reachability.
pub async fn is_mihomo_running() -> bool {
    let mut url = manager::controller_url();
    if url.is_empty() {
        if let Ok(cm) = mihomo_rs::ConfigManager::with_home(dirs::data_dir()) {
                ...
                if let Ok(content) = tokio::fs::read_to_string(&config).await {
                    if let Ok(val) = serde_yaml::from_str::<serde_yaml::Value>(&content) {
                        if let Some(addr) = val.get("external-controller").and_then(|v| v.as_str())
                        {
                            ...
                            manager::set_controller_url(url.clone());
                            let _ = api::init_client(&url, read_secret_from_config(&config));
                        }
                    }
                }
            }
...
pub async fn service_status() -> Result<String, String> {
    ...
    #[cfg(not(windows))]
    {
        if !manager::core_installed().await {
...
pub async fn test_service_connection() -> Result<bool, String> {
    #[cfg(windows)]
    {
        ...
        let req = crate::backend::service_host::IpcRequest::Ping;
...
pub async fn install_service() -> Result<(), String> {
    ...
    #[cfg(not(windows))]
    {
        manager::install_core().await.map_err(|e| e.to_string())
    }
}
...
pub async fn uninstall_service() -> Result<(), String> {
    ...
    #[cfg(not(windows))]
    {
        manager::stop_core().await.map_err(|e| e.to_string())
    }
}
