...
impl Sysproxy {
    ...
    /// Mirrors the Windows signature; Linux has no per-connection (RAS) proxy.
    #[inline]
...
fn parse_kde_proxy(schema: &str, service: &str) -> Result<(String, u16)> {
    ...
    if schema.is_empty() {
        // KDE's default kioslaverc may omit per-scheme entries; empty means unset.
        return Ok(("".into(), 0));
    }
